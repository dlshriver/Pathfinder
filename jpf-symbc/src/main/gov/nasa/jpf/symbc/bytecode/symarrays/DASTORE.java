...
public class DASTORE extends gov.nasa.jpf.jvm.bytecode.DASTORE {
    ...
    public Instruction execute(ThreadInfo ti) {
        ...
        PCChoiceGenerator temp_cg = (PCChoiceGenerator) ti.getVM()
                .getLastChoiceGeneratorOfType(PCChoiceGenerator.class);
        if (temp_cg != null) {
            if (temp_cg.getCurrentPC().arrayExpressions
                    .containsKey(ti.getElementInfo(ti.getModifiableTopFrame().peek(3)).toString())) {
                ti.getModifiableTopFrame().setOperandAttr(3, temp_cg.getCurrentPC().arrayExpressions
                        .get(ti.getElementInfo(ti.getModifiableTopFrame().peek(3)).toString()));
            }
        }
        ...
        assert (indexAttr != null) : "indexAttr shouldn't be null in DASTORE instruction";
        ...
        if (peekArrayAttr(ti) == null || !(peekArrayAttr(ti) instanceof ArrayExpression)) {
            ...
            if (peekIndexAttr(ti) == null || !(peekIndexAttr(ti) instanceof IntegerExpression)) {
                ...
                for (int i = 0; i < arrayInfo.arrayLength(); i++) {
                    ...
                    pc._addDet(Comparator.EQ, new SelectExpression(arrayAttr, new IntegerConstant(i)),
        ...
        assert (arrayAttr != null) : "arrayAttr shouldn't be null in DASTORE instruction";
        ...
        if ((Integer) cg.getNextChoice() == 1) { // check bounds of the index
            ...
            if (pc.simplify()) { // satisfiable
                ...
                return ti.createAndThrowException("java.lang.ArrayIndexOutOfBoundsException",
                        "index greater than array bounds");
            } else {
            ...
            if (pc.simplify()) { // satisfiable
                ((PCChoiceGenerator) cg).setCurrentPC(pc);
                return ti.createAndThrowException("java.lang.ArrayIndexOutOfBoundsException",
                        "index smaller than array bounds");
            } else {
                ti.getVM().getSystemState().setIgnored(true);
                return getNext(ti);
            }
